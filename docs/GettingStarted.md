...
sqldep is based on a simple premise: you create a directory structure on disk with well-known folder names, and then add `.sql` files.  sqldep processes these folders in a fixed order, and runs the scripts inside in alphabetical order.  That's it!
...
**Everytime Scripts**
These scripts are run (you guessed it) **every time** sqldep executes :)  More info [here](ScriptTypes/EverytimeScripts.md)

## Directory run order

By default, sqldep processes the files in a standard set of directories in a fixed order for deterministic processing.  
As of sqldep 1.4, the folder structure is [fully customisable](ConfigurationOptions/FolderConfiguration.md). However,
...
This directory is only processed if the database was created from scratch by sqldep.  Maybe you need to add user accounts or similar?
...
Note that there's no `down`!  If you've dropped a column in your `up` scripts, how do you possibly script an undo for that operation?  sqldep has support for running the migration inside a transaction, and will rollback on script failure.
