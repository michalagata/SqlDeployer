...
## Notes
⚠ Using token replacement in your scripts locks those scripts into sqldep. I would use something like this VERY sparingly.

## Supported Tokens
The curently supported tokens are defined in the [`TokenProvider`]({{ site.github.repository_url }}/blob/main/sqldep/Infrastructure/TokenProvider.cs) class.  While not all the RoundhousE tokens are supported yet, this set of allowed tokens should improve over time (PR's welcome!).
...
- If you pass clearly invalid info on the commandline (eg missing the equal sign) then sqldep will terminate with an error.
