...
If you name a sql file with `*.ENV.*`, sqldep will start looking for the environment name somewhere on the file. If it doesn't find the proper environment, it will not run the file.  
...
You configure the environment sqldep is targeting via the `--env` or `--environment` command line option, eg `> sqldep --env=LOCAL`.
...
Roundhouse supported a single _migration_ targeting multiple environments, however sqldep currently does not allow this, i.e. at most one `--env` argument is allowed.  If this causes you pain please raise an issue to discuss!

## Does this lock me into sqldep?
