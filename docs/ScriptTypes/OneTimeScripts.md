...
sqldep always runs files in order alphabetically.
...
## Errors
If there is a change to a one time script and the migrator is run, sqldep will determine you have changed that file and will shut down immediately with errors.  That being said, there is a configuration setting to allow you to still run with warnings. Although not recommended, sqldep tries not to be a tool that constrains users.  
