...
The folder configuration in sqldep is now (since SqlDeployer v 1.4) entirely configurable. That means you can 
specify the folder structure of choice, and make sqldep work with your existing structure, rather than
having to change the folder structure to suite how sqldep works. There is, however, a [default configuration](#default-folder-configuration),
which might be a good starting point if you have no special requirements.

## Folder types

sqldep works with three different folder types:
...
| Everytime Scripts | EveryTime | These scripts are run (you guessed it) **every time** sqldep executes :) |  [Everytime scripts](../ScriptTypes/EverytimeScripts.md#everytime-scripts) |

## Specifying a custom folder configuration

If you want to specify a custom folder configuration, you have two options:

1. Start with the [default sqldep folder configuration](#default-folder-configuration), and make adjustments
1. Specify a fully customised set of folders to use

**Note:** you can either:
1) specify the parameters directly on the command line, or 
2) _supply a filename_, and sqldep will read the folder config from that file.
...
to the `--folders` argument, and sqldep will run them in the specified order, with the specified configuration for 
each folder.

The list of folder should consist of some keys that are not among the default folders. Then none of the default folders will be configured, just the ones you supply. If all the keys are some of the default ones, e.g. only `up`, `views` and `sprocs`, sqldep will assume that you want the default configuration, with the adjustments you have specified.
...
sqldep interprets it as a folder name.


## Default folder configuration

Out of the box, sqldep has a suggested folder structure. If you start from zero, this might be a good starting point. 
Even if you choose to go with the default folder configuration, you can (as of sqldep 1.4) configure the names of the default
...
An example, if you want to use a folder `tables` to keep you `up` scripts in, use the following argument to sqldep:

```bash
$ sqldep --folders up=tables
```



sqldep processes the files in a standard set of directories in a fixed order for deterministic processing.  Folders are run in the following order:
...
| <nobr>3. runAfterCreateDatabase</nobr> | Anytime scripts | This directory is only processed if the database was created from scratch by sqldep.  Maybe you need to add user accounts or similar?
| <nobr>4. runBeforeUp</nobr> | Anytime scripts | 
| <nobr>5. up</nobr> | One-time scripts | This is where the bulk of your 'migration' scripts end up, eg adding tables, removing columns, adding reference data etc. <br><br> Note that there's no `down`!  If you've dropped a column in your `up` scripts, how do you possibly script an undo for that operation?  sqldep has support for running the migration inside a transaction, and will rollback on script failure.
